...
import { getPopups, createPopup, updatePopup, deletePopup } from '../../services/popupService';
...
CREATE TABLE IF NOT EXISTS public.popups (
  ...
  show_every_time BOOLEAN DEFAULT false,
...
-- Allow authenticated users to manage popups
CREATE POLICY "Allow authenticated users to manage popups"
  ON public.popups FOR ALL
  USING (auth.role() = 'authenticated');

-- Create storage bucket for popups
INSERT INTO storage.buckets (id, name, public) 
VALUES ('popups', 'popups', true)
ON CONFLICT (id) DO NOTHING;

-- Storage policies
CREATE POLICY "Public Access for popups bucket"
ON storage.objects FOR SELECT
USING ( bucket_id = 'popups' );

CREATE POLICY "Auth Insert for popups bucket"
ON storage.objects FOR INSERT
WITH CHECK ( bucket_id = 'popups' AND auth.role() = 'authenticated' );

CREATE POLICY "Auth Update for popups bucket"
ON storage.objects FOR UPDATE
USING ( bucket_id = 'popups' AND auth.role() = 'authenticated' );

CREATE POLICY "Auth Delete for popups bucket"
...
export default function Popups() {
  ...
  const fetchPopups = async () => {
    try {
      ...
      if (err.message && (err.message.includes('Could not find the table') || err.message.includes('relation "public.popups" does not exist'))) {
        setDbError(true);
      } else {
  ...
  const openModalForEdit = (popup) => {
    ...
    setTitle(popup.title_en || popup.title || '');
    setDescription(popup.description_en ? popup.description_en : (popup.description || ''));
    setStatus(popup.status);
    setShowEveryTime(popup.show_every_time || false);
    setShowDetails(popup.show_details ?? true);
    setImageFile(null);
  ...
  const uploadImage = async (file) => {
    ...
    if (uploadError) {
      if (uploadError.message.includes("Bucket not found")) {
        throw new Error("Storage bucket 'popups' not found. Please run the SQL script to create it.");
      }
      throw uploadError;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const descTa = description ? await translateText(description) : '';
    ...
    return (
      <div style={{ padding: '2rem', background: '#fff', borderRadius: '8px', border: '1px solid #fecaca' }}>
        ...
        <p style={{ marginBottom: '1rem', color: '#475569' }}>
          The <b>popups</b> table or storage bucket was not found in your Supabase project.
                ...
                {paginatedData.map((popup) => (
                  <div className={styles.popupGridRow} key={popup.id}>
                    ...
                    <div className={styles.gridCellContent}>
                      ...
                      {popup.image_url ? (
                        <img src={popup.image_url} alt={popup.title_en} className={styles.gridPopupThumb} />
                      ) : (
                      ...
                      <div className={styles.gridPopupText}>
                        <div className={styles.gridPopupTitle}>{popup.title_en}</div>
                        <div className={styles.gridPopupSub}>{(popup.description_en || popup.description || '')?.substring(0, 30)}...</div>
                      </div>
                    </div>
                    ...
                    <div className={styles.gridCellPreview}>
                      <div className={styles.quoteBlock}>
                        ...
                        <div className={styles.quoteText}>
                          {(popup.description_en || popup.description || '')?.substring(0, 70)}...
                        </div>
                    ...
                    <div className={styles.settingCard}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <label className={styles.customCheckboxLabel}>
                          ...
                          <span style={{ fontWeight: 600, color: '#0f172a' }}>Show</span>
                        </label>
                      </div>
                    </div>
                  </div>
